...
function startGame(){
    ...
    tablero = new Tablero(tableroLeftPosition, 0, tableroWidth, tableroHeight, 10, ctx, fondo, radioCasilla, cvs, largoLinea);
    tablero.draw();
    p1 = new Jugador(cantFicha, fichaSelectedP1, tableroLeftPosition/2, radioCasilla, ctx, "Franco");
    p2 = new Jugador(cantFicha, fichaSelectedP2, posInicialP2, radioCasilla, ctx, "Valentin");
    p1.drawFichas();
    p2.drawFichas();
...
//mejorar interface
    // -hayGanador, no timer, no fichas
    // opcion de largo de linea (4, 5, 6 en linea)
//dar opcion de reiniciar/game over cuando:
    // -no hay fichas
    // -no time
    // -hay ganador
    ...
    // El juego se reiniciara cuando ambos jugadores se queden sin fichas, si se acaba el tiempo o si hay un ganador
});